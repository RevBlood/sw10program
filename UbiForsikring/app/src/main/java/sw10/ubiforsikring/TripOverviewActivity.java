...
public class TripOverviewActivity extends AppCompatActivity {
    Context mContext;
    ArrayAdapter mTripOverviewAdapter;
    List<TripOverviewEntry> mTripList;

    //Active Trip
    LatLng mPreviousPosition;
    TripOverviewEntry mActiveTrip;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mTripList.add(0, new TripOverviewEntry(1, 1010101010, 1011111111, 10700, 47, 16.01));
        mTripList.add(0, new TripOverviewEntry(2, 1000101111, 1011110101, 10700, 35, 14.48));
        mTripList.add(0, new TripOverviewEntry(3, 1001101010, 1111111111, 7200, 25, 11.11));
        mTripList.add(0, new TripOverviewEntry(4, 1010101110, 1110000001, 4300, 13, 5.84));
        mTripList.add(0, new TripOverviewEntry(5, 1000100010, 1001110101, 5600, 4, 6.13));
        mTripList.add(0, new TripOverviewEntry(6, 100000000, 110000000, 20100, 10, 22.7));
        mTripList.add(0, new TripOverviewEntry(7, 10000000, 11000000, 20100, 39, 28.34));

        //Setup ListView
        ListView mainListView = (ListView) findViewById(R.id.TripOverviewListView);
        mTripOverviewAdapter = new TripOverviewAdapter(this, mTripList);
        mainListView.setAdapter(mTripOverviewAdapter);
    ...
    ListView.OnItemClickListener MainListViewListener = new AdapterView.OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
            ...
            int itemType = mTripOverviewAdapter.getItemViewType(position);

            if(itemType == TripOverviewAdapter.VIEWTYPE_CURRENT) {
    ...
    private void HandleTripStatus() {
        if (mIsTripActive) {
            //Update the ListView
            mActiveTrip = new TripOverviewEntry(true, false);
    ...
    private void HandleProcessingStatus() {
        if (mIsProcessing) {
            mActiveTrip = new TripOverviewEntry(false, true);
